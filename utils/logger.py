...
class Logger:
    log_moment = str(datetime.datetime.now().strftime("%Y-%m-%d_%H-%M-%S"))
    file_name = f".\\logs\\log_{log_moment}.log"

    @classmethod
    def write_log_to_file(cls, data: str):
        with open(file=cls.file_name, mode='a', encoding='utf=8') as logger_file:
